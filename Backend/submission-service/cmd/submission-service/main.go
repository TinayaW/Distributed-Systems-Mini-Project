...
import (
	...
	"backend/submission-service/internal/consul"
	"backend/submission-service/internal/postgre"
	"backend/submission-service/internal/router"
...
func startServer(configdata *config.Config, consulClient *api.Client, db *sql.DB) {
	router := router.SetupRouter(consulClient, db, configdata)
